...
if($id){
    ...
    $stmt = $conn->prepare("SELECT * FROM printer WHERE id = ?");
...
// Merk List kategori Printer
$merk_list = mysqli_query($conn,"SELECT * FROM merk_asset WHERE category='Printer' ORDER BY merk_name");
...
if($merk_id_exists){
    // UBAH: Gunakan prepared statement
...
<html>
<head>
    <title><?= $id ? 'Edit' : 'Input' ?> Printer Asset</title>
    ...
    <style>
        ...
        .checkbox-group {
            ...
            padding: 10px;
            min-height: 38px; /* Samakan tinggi dengan form-control-sm */
            display: flex;
            align-items: center;
        }
        .card-header {
        ...
        <div class="col-lg-10">
            <div class="card shadow-sm">
                <div class="card-header">
                    <h2 class="h5 mb-0"><?= $id ? 'Edit' : 'Input' ?> Printer Asset</h2>
                </div>
                <div class="card-body">
                    <form method="POST" action="printer_process.php">
                        <?php if($id): ?>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <select name="model" id="model" class="form-select form-select-sm" required>
                                    ...
                                    foreach(['InkJet','LaserJet','Photocopy','Ribbon'] as $c): ?>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label">Port Connection</label>
                                <div class="checkbox-group">
                                    <?php
                                    $port_selected = explode(',', isset($data['port_connection']) ? $data['port_connection'] : '');
                                    foreach(['LAN','USB','WiFi'] as $p):
                                    ?>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="checkbox" name="port_connection[]" value="<?= $p ?>" id="port_<?= $p ?>" <?= in_array($p,$port_selected)?'checked':'' ?>>
                                            <label class="form-check-label" for="port_<?= $p ?>"><?= $p ?></label>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        ...
                        <?php
                        $port_selected_js = isset($data['port_connection']) ? $data['port_connection'] : '';
                        $has_network_port = !empty($port_selected_js) && (in_array('LAN', explode(',', $port_selected_js)) || in_array('WiFi', explode(',', $port_selected_js)));
                        ?>
                        <div class="row mb-3">
                            <div class="col-md-6" id="mac_wrapper" style="<?= $has_network_port ? '' : 'display:none;' ?>">
                        ...
                        <div class="mt-4 border-top pt-3 d-flex gap-2">
                            ...
                            ?>
                            <a href="printer.php?<?= $cancel_query ?>" class="btn btn-outline-secondary">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
...
(function() {
    ...
    const portCheckboxes = document.querySelectorAll('input[name="port_connection[]"]');
    
    function updateMacVisibility() {
        const selected = Array.from(portCheckboxes)
                              .filter(cb => cb.checked)
                              .map(cb => cb.value);
                              
        const macWrapper = document.getElementById('mac_wrapper');
        const macField = document.getElementById('mac_address');
        
        if(selected.includes('LAN') || selected.includes('WiFi')){
            macWrapper.style.display = 'block';
        } else {
            if(macField) macField.value = '';
            macWrapper.style.display = 'none';
        }
    }
    
    portCheckboxes.forEach(function(cb){
        cb.addEventListener('change', updateMacVisibility);
    });
