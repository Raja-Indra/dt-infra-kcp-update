...
$error = '';
$success = '';
if(isset($_GET['added'])) $success = "Server asset berhasil ditambahkan.";
if(isset($_GET['updated'])) $success = "Server asset berhasil diperbarui.";
if(isset($_GET['deleted'])) $success = "Server asset berhasil dihapus.";
...
$sql_total = "
    ...
    FROM server s
    LEFT JOIN merk_asset m ON s.merk_id = m.id
    LEFT JOIN type_asset t ON s.type_id = t.id
    $where_sql
";
...
$sql = "
    SELECT s.*, m.merk_name, t.type_name
    FROM server s
    LEFT JOIN merk_asset m ON s.merk_id = m.id
    LEFT JOIN type_asset t ON s.type_id = t.id
    $where_sql
...
// Get merk list
$merk_list = mysqli_query($conn,"SELECT id, merk_name FROM merk_asset WHERE category='Server' ORDER BY merk_name");
...
<div class="container-fluid mt-4">
    <div class="card shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
            <h2 class="h5 mb-0">Server Assets</h2>
        </div>
        <div class="card-body">
                ...
                <div class="row g-2 align-items-end">
                    <div class="col-md-3">
                        ...
                        <select name="model" id="model" class="form-select form-select-sm">
                            ...
                            <?php foreach(['Rack Mount','Tower'] as $c): ?>
                    ...
                    <div class="col-auto">
                        <a href="server.php" class="btn btn-outline-secondary btn-sm">Reset</a>
                    </div>
                    <div class="col-auto">
                        <a href="server_input.php" class="btn btn-success btn-sm">+ Input Asset</a>
                    </div>
                ...
                <table class="table table-sm table-bordered table-striped table-hover">
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($row['storage_capacity']) ?></td>
                            <td><?= htmlspecialchars($row['storage_type']) ?></td>
                            <td><?= htmlspecialchars($row['mac_address']) ?></td>
                            <td><?= htmlspecialchars($row['psu']) ?></td>
                            <td><?= htmlspecialchars($row['raid_system']) ?></td>
                            <td><?= htmlspecialchars($row['virtualization']) ?></td>
                            ...
                            <td class="text-center text-nowrap">
                                ...
                                ?>
                                <a href="server_input.php?<?= $link_params ?>" class="btn btn-primary btn-sm">Edit</a> 
                                <a href="server_delete.php?<?= $delete_params ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this asset?')">Delete</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                    </tbody>
