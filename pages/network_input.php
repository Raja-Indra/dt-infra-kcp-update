...
if($id){
    ...
    $stmt = $conn->prepare("SELECT * FROM network WHERE id = ?");
...
// Merk list (hanya Network)
$merk_list = mysqli_query($conn, "SELECT * FROM merk_asset WHERE category='Network' ORDER BY merk_name");
...
<html>
<head>
    <title><?= $id ? 'Edit' : 'Input' ?> Network Asset</title>
        ...
        <div class="col-lg-10">
            <div class="card shadow-sm">
                <div class="card-header">
                    <h2 class="h5 mb-0"><?= $id ? 'Edit' : 'Input' ?> Network Asset</h2>
                </div>
                <div class="card-body">
                    <form method="POST" action="network_process.php">
                        <?php if($id): ?>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <select name="category" id="category" class="form-select form-select-sm" required>
                                    ...
                                    <?php 
                                    $category_val = isset($data['category']) ? $data['category'] : '';
                                    foreach(['Router','Switch','Wireless'] as $c): ?>
                                        <option value="<?= $c ?>" <?= $category_val == $c ? 'selected' : ''?>><?= $c ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="merk_id" id="merk_select" class="form-select form-select-sm" required>
                                    ...
                                    mysqli_data_seek($merk_list, 0); // Reset pointer
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <select name="condition" id="condition" class="form-select form-select-sm">
                                    <option value="">Select</option>
                                    <?php $condition_val = isset($data['condition']) ? $data['condition'] : 'Good'; // Default Good ?>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="status" id="status" class="form-select form-select-sm">
                                    <option value="">Select</option>
                                    <?php 
                                    $status_val = isset($data['status']) ? $data['status'] : 'Installed'; // Default Installed
                        ...
                        <div class="mt-4 border-top pt-3 d-flex gap-2">
                            ...
                            ?>
                            <a href="network.php?<?= $cancel_query ?>" class="btn btn-outline-secondary">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
// Dynamic Type (Tidak ada perubahan, sudah benar)
